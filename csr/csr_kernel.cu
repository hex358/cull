...
__device__ __forceinline__ int radial_to_bin(double r, double inv_radial_bin_size, int radial_bin_count) {
    int b = (int)floor(r * inv_radial_bin_size);
    return clamp_int(b, 0, radial_bin_count - 1);
}
...
__global__ void prepare_emit_entries_kernel(
    ...
    double inv_radial_bin_size, int radial_bin_count,
    ...
    int32_t* __restrict__ rb_min_arr, int32_t* __restrict__ rb_max_arr,
    ...
    if (slab >= slabs) {
        ...
        rb_min_arr[ri] = -1; rb_max_arr[ri] = -1;
        return;
    }
    ...
    int rb0 = radial_to_bin(rmin_phys, inv_radial_bin_size, radial_bin_count);
    int rb1 = radial_to_bin(rmax_phys, inv_radial_bin_size, radial_bin_count);
    if (rb1 < rb0) { int t = rb0; rb0 = rb1; rb1 = t; }
    rb_min_arr[ri] = rb0;
    rb_max_arr[ri] = rb1;
    ...
    int cart_cells_touched = (x1 - x0 + 1) * (y1 - y0 + 1) * (z1 - z0 + 1);
    int radial_cells_touched = rb1 - rb0 + 1;
    int cells_touched = cart_cells_touched * radial_cells_touched;
    ...
    for (int rb = rb0; rb <= rb1; ++rb) {
        uint64_t slab_radial_base = ((uint64_t)batch_slab * (uint64_t)radial_bin_count + (uint64_t)rb) * cell_count;
        for (int ix = x0; ix <= x1; ++ix) {
            for (int iy = y0; iy <= y1; ++iy) {
                for (int iz = z0; iz <= z1; ++iz) {
                    ...
                    entry_keys[entry] = slab_radial_base + cid;
...
__global__ void cell_owned_pairs_kernel(
    ...
    uint64_t cell_count, int cells_per_axis, int radial_bin_count, double screen_radius_sq,
    ...
    const int32_t* __restrict__ rb_min_arr, const int32_t* __restrict__ rb_max_arr,
    ...
    uint64_t slab_radial = key / cell_count;
    uint64_t cid = key - slab_radial * cell_count;
    int batch_slab = (int)(slab_radial / (uint64_t)radial_bin_count);
    int radial_bin = (int)(slab_radial - (uint64_t)batch_slab * (uint64_t)radial_bin_count);
    ...
    for (int a = threadIdx.x; a < count; a += blockDim.x) {
        ...
        for (int b = a + 1; b < count; ++b) {
            ...
            int rb0_i = rb_min_arr[ri];
            int rb0_j = rb_min_arr[rj];
            if (rb0_i < 0 || rb0_j < 0) continue;
            int first_shared_rb = max(rb0_i, rb0_j);
            if (radial_bin != first_shared_rb) continue;
...
CullGridResult run_csr(
    ...
    if (n < 2 || t_count < 2) {
        ...
        result.stats.method = "CULL-GRID-CUDA-CSR-RADIALKEY-V19";
    ...
    const double radial_bin_size_km = std::max(256.0, 0.5 * grid_cell_size_km);
    const double inv_radial_bin_size = 1.0 / radial_bin_size_km;
    const double radial_bound_km = std::sqrt(3.0) * grid_bound_km + expand_km + radial_bin_size_km;
    const int radial_bin_count = (int)std::ceil(radial_bound_km / radial_bin_size_km);
    const int64_t total_pair_slabs = (int64_t)slabs * n * (n - 1) / 2;

    double t_total0 = now_s();

    int32_t *d_xmin, *d_xmax, *d_ymin, *d_ymax, *d_zmin, *d_zmax, *d_rbmin, *d_rbmax;
    ...
    CUDA_CHECK(cudaMalloc(&d_rbmin, batch_prim_count * sizeof(int32_t)));
    CUDA_CHECK(cudaMalloc(&d_rbmax, batch_prim_count * sizeof(int32_t)));
    ...
    for (int batch_start = 0; batch_start < slabs; batch_start += batch_slabs) {
        ...
        prepare_emit_entries_kernel<<<current_blocks, threads_per_block>>>(
            ...
            inv_radial_bin_size, radial_bin_count,
        ...
        cell_owned_pairs_kernel<<<unique_count, block_pairs>>>(
            ...
            d_objs,
            n, slabs, batch_start,
            cell_count, cells_per_axis, radial_bin_count, screen_radius_km * screen_radius_km,
            d_xmin, d_xmax, d_ymin, d_ymax, d_zmin, d_zmax,
            d_rbmin, d_rbmax,
            d_rmin, d_rmax,
    ...
    CUDA_CHECK(cudaFree(d_rbmin)); CUDA_CHECK(cudaFree(d_rbmax));
    ...
    CullGridStats st;
    st.method = "CULL-GRID-CUDA-CSR-RADIALKEY-V19";
    st.n_used = n;
    st.slabs = slabs;
    ...
    st.grid_radial_bin_size_km = radial_bin_size_km;
    st.grid_radial_bin_count = radial_bin_count;
    st.grid_radial_bound_km = radial_bound_km;
